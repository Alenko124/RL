...
class ApolloLunarAgent(Agent):
    ...
    def pi(self, x, k, info=None):
        ...
        if k == 0:
            ...
            self.pid_alt = PID(dt=self.dt, Kp=self.Kp_altitude, Kd=0, Ki=0, target=np.abs(x[0]))
            self.pid_ang = PID(dt=self.dt, Kp=self.Kp_angle, Kd=0, Ki=0, target=(.25*np.pi)*(x[0]+x[2]))
        ...
        alt_error = (np.abs(x[0]) - x[1])
        ang_error = ((.25*np.pi)*(x[0]+x[2]) - x[4])
        # Use PID to get adjustments
        alt_adj = self.Kp_altitude*alt_error - self.Kd_altitude*x[3]
        ang_adj = self.Kp_angle*ang_error - self.Kd_angle*x[5]
        #alt_adj = self.pid_alt.pi(x[1]) + self.Kd_altitude*x[3]
        #ang_adj = self.pid_ang.pi(x[4]) + self.Kd_angle*x[5]
