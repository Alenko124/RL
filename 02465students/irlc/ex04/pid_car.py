...
class PIDCarAgent(Agent):
    def __init__(self, env: ControlEnvironment, v_target=1.0, use_both_x5_x3=True):
        """
        ...
        #----------------------------------------------------------------------------------------------------------------------------
        self.pid_angle = PID(dt=env.discrete_model.dt, Kp=40.0, Ki=0.0, Kd=10.0, target=0) 
        self.pid_velocity = PID(dt=env.discrete_model.dt, Kp=30.0, Ki=0.0, Kd=0.0, target=v_target)
    ...
    def pi(self, x, k, info=None):
        ...
        u1=self.pid_angle.pi(x[5])
        u2=self.pid_velocity.pi(x[0])
        return [u1, u2]
