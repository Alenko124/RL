...
class PID:
    ...
    def pi(self, x):
        ...
        e= self.target - x # Compute the error (the difference between the target and the current state).
        self.I += e * self.dt # Update the integral term (you can use self.I
        D = (e - self.e_prior) / self.dt # Compute the derivative term (you can use self.e_prior)
        u = self.Kp * e + self.Ki * self.I + self.Kd * D # Compute the control action using the PID formula.
        self.e_prior = e # Update
        return u
...
def pid_explicit():
    ...
    for _ in range(200): # Simulate for 200 steps, i.e. 0.05 * 200 seconds.
        ...
        u = pid.pi(x_cur[0]) # Compute the control action using the PID controller. Note that you only need to pass the position (the first element of x_cur) to the PID controller.
    ...
    savepdf("pid_basic")
    plt.savefig('pid_basic.png', dpi=300, bbox_inches='tight')
