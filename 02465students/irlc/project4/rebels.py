...
class UCBAgent(QAgent):
    def __init__(self, env, gamma=1, alpha=0.1, c=5):
    ...
    def pi(self, s, k, info=None):
        ...
        actions = [a for a, allowed in enumerate(info["mask"]) if allowed]

        # Try all actions once in order
        for a in actions:
            if self.N[s, a] == 0:
                return a

        t = sum(self.N[s, a] for a in actions)

        values = []

        for a in actions:
            bonus = self.c * np.sqrt(np.log(t) / self.N[s, a])
            values.append(self.Q[s, a] + bonus)
    ...
    def train(self, s, a, r, sp, done=False, info_s=None, info_sp=None):
        ...
        target = r + self.gamma * (0 if done else self.Q[sp, a_star])
...
def get_ucb_actions(layout : list, alpha : float, c : float, episodes : int, plot=False) -> list: 
    ...
    _, trajectories = train(env, agent, num_episodes=episodes)

    actions = []

    for traj in trajectories:

        # Remove dummy terminal action
        actions.extend(traj.action[:-1])
