...
def boeing_simulation():
    ...
    #----------------------------------------------------------------------------------------------------------------------------
    agent = LQRAgent(env, A=A, B=B, Q=Q, R=R, d=d, q=q)
...
def compute_Q_R_q(model : ControlModel, dt : float):
    ...
    Q = cost.Q * dt
    R = cost.R * dt
    q = cost.q * dt if cost.q is not None else None
...
def compute_A_B_d(model : ControlModel, dt : float):
    ...
    B_discrete = np.linalg.solve(model.A, (A_discrete - I)) @ model.B
    d_discrete = np.linalg.solve(model.A, (A_discrete - I)) @ d
