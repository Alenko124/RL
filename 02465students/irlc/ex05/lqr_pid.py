...
class ConstantLQRAgent(LQRAgent):
    ...
    def __init__(self, env, A, B, Q, R, d=None, q=None):
        super().__init__(env, A, B, Q, R, d=d, q=q)

        # Uzmi jedan konstantan gain (najčešće prvi ili zadnji)
        self.L_const = self.L[0]
        self.l_const = self.l[0]

    def pi(self, x, k=None, info=None):
        return self.L_const @ x + self.l_const
