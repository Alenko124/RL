...
def f_euler(x : np.ndarray, u : np.ndarray, Delta=0.05) -> np.ndarray: 
    ...
    return np.array([
        x[0] + Delta * u[0] * np.cos(x[2]),
        x[1] + Delta * u[0] * np.sin(x[2]),
        x[2] + Delta * u[1]
    ])
...
def linearize(x_bar, u_bar, Delta=0.05):
    ...
    # 1. Kreiraj environment (ovo automatski pravi diskretni model)
    env = R2D2Environment(dt=Delta)
    model = env.discrete_model  # ovo je DiscreteControlModel

    # 2. Uzmi Jacobiane
    A, B = model.f_jacobian(x_bar, u_bar)

    # 3. Izračunaj d
    f_bar = model.f(x_bar, u_bar)
    d = f_bar - A @ x_bar - B @ u_bar
...
def drive_to_linearization(x_target, plot=True): 
    ...
    env = R2D2Environment(dt=dt, x_target=x_target)
    ...
    _, trajectories = train(env, agent, num_episodes=1, return_trajectory=True, reset=True)  # Note reset=False to maintain initial conditions.

    if plot:
        plot_trajectory(trajectories[0], env, xkeys=[0,1,2], ukeys=[0,1])
    #env.close()
    return np.array(trajectories[0].state)
...
class MPCAgent(Agent):
    def __init__(self, env, model):
        self.model = model
        self.N = 50
        self.ubar = np.zeros(env.action_space.shape)  # This will be updated in each step. Initial guess is zero.
        super().__init__(env)

    def pi(self, x, k, info=None):
        # 1. Linearizacija oko trenutnog stanja
        A, B = self.model.f_jacobian(x, self.ubar)
        d = self.model.f(x, self.ubar) - A @ x - B @ self.ubar

        Q, q, R = self.model.cost.Q, self.model.cost.q, self.model.cost.R

        # 2. LQR
        (L, l), _ = LQR(A=[A]*self.N, B=[B]*self.N,
                        Q=[Q]*self.N, q=[q]*self.N,
                        R=[R]*self.N, d=[d]*self.N)
...
def drive_to_mpc(x_target, plot=True) -> np.ndarray: 
    ...
    agent = MPCAgent(env, model=env.discrete_model)

    _, trajectories = train(env, agent, num_episodes=1, return_trajectory=True, reset=True)
